...
> ⚠️ **Status: V20.0.3 (dev build).** The panel + host script are implemented and talk to a hosted relay (moonlit.onl) for room routing, sync relay, and file transfer. Sync is fully **mutual** — every peer in the room is equal, no host-only privileges. This is a debug/unsigned build, see [Installation](#installation).
